...
pdf('plots/chl_npp_comparison_giop.pdf',height=10,width=8)
...
##--GIOP--################
pdf('plots/chl_vs_c_time_series_giop.pdf',height=6,width=8)
par(mfrow=c(6,1),mar=c(1,2,1,2),oma=c(2,3,2,3),cex.axis=0.7)
for(i in 6:1){
  chli <- latmean(CHL_GIOP,loni=loni,lati=LAT[[i]])
...
par(mfrow=c(6,1),mar=c(1,2,1,2),oma=c(2,3,2,3),cex.axis=0.7)
for(i in 6:1){
  chli <- latmean(CHL_GSM,loni=loni,lati=LAT[[i]])
...
par(mfrow=c(3,2),mar=c(2,1,1,3.5),oma=c(3,3,2,3),xpd=FALSE,cex.axis=0.8)
for(i in 6:1){ 
...
##--GSM--#############################
pdf('plots/CHLC_vs_I_gsm.pdf',height=6,width=6)
par(mfrow=c(3,2),mar=c(2,1,1,3.5),oma=c(3,3,2,3),xpd=FALSE,cex.axis=0.8)
for(i in 6:1){ 
...
k <- 1
for(i in 6:1){
  if(i==6){chllims=c(0,2)}else{chllims=c(0,0.5)}
  if(i==6){clims=c(0,70)}else{clims=c(0,40)}
  mldi <- latmean(MLD,loni=loni,lati=LAT[[i]])
  ci   <- latmean(CARBON_GIOP,loni=loni,lati=LAT[[i]])
  chli <- latmean(CHL_GIOP,loni=loni,lati=LAT[[i]])
	plot(mldi,ci,pch=19,cex=0.25,xlim=c(0,150),ylim=clims,xaxt='n',xlab='',ylab='')
		linep(mldi,ci,mldin)
		if(i==1) axis(1)
		if(i==3) mtext(side=2,expression('Carbon Concentration [mg/m'^3*']'),line=2,adj=0.335)
		mtext(adj=0,cex=0.6,labs[k])
		k=k+1
	plot(mldi,chli,pch=19,cex=0.25,xlim=c(0,150),ylim=chllims,xaxt='n',xlab='',ylab='')
...
k <- 1
for(i in 6:1){
  if(i==6){chllims=c(0,2)}else{chllims=c(0,0.6)}
  if(i==6){clims=c(0,70)}else{clims=c(0,40)}
  mldi <- latmean(MLD,loni=loni,lati=LAT[[i]])
  ci   <- latmean(CARBON_GSM,loni=loni,lati=LAT[[i]])
  chli <- latmean(CHL_GSM,loni=loni,lati=LAT[[i]])
