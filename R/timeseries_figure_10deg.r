...
chl=chl_giop=chl_gsm=mld=carbon=chlc=mu=par=kd=e=vgpm=cbpm=cafe=epvgpm <- list()
for(i in 1:6){
	...
	tmpc                    <- CARBON[loni,LAT[[i]],]
	carbon[[names(LAT)[i]]] <- apply(tmpc,3,function(x) mean(x, na.rm=TRUE))
	
	chlc[[names(LAT)[i]]] <- chl[[names(LAT)[i]]]/carbon[[names(LAT)[i]]] 
	
  tmpcbpm                <- CBPM[loni,LAT[[i]],]
	cbpm[[names(LAT)[i]]]  <- apply(tmpcbpm,3,function(x) mean(x,na.rm=TRUE))
	...
	tmpepvgpm              <- EP_VGPM[loni,LAT[[i]],]
  epvgpm[[names(LAT[i])]]<- apply(tmpepvgpm,3,function(x) mean(x,na.rm=TRUE))
			
	tmppar                <- PAR[loni,LAT[[i]],]
	par[[names(LAT)[i]]]  <- apply(tmppar,3,function(x) mean(x, na.rm=TRUE))
	
	tmpkd                 <- KD[loni,LAT[[i]],]
	kd[[names(LAT)[i]]]   <- apply(tmpkd,3,function(x) mean(x, na.rm=TRUE))
	
	e[[names(LAT)[i]]]    <- (1/(kd[[names(LAT)[i]]]*mld[[names(LAT)[i]]]))*par[[names(LAT)[i]]]*exp(-(kd[[names(LAT)[i]]]*mld[[names(LAT)[i]]]))*(1-exp(-(kd[[names(LAT)[i]]]*mld[[names(LAT)[i]]])))
}
...
plot(time,cbpm[[6]],type='l',col=cols[1],xaxt='n',yaxt='n',xlim=timedims,ylim=range(c(cbpm[6],cafe[6],vgpm[6],epvgpm[6]),na.rm=TRUE))
lines(time,vgpm[[6]],type='l',col=cols[2],xaxt='n',yaxt='n')
lines(time,cafe[[6]],type='l',col=cols[3],xaxt='n',yaxt='n')
lines(time,epvgpm[[6]],type='l',col=cols[4],xaxt='n',yaxt='n')
legend('topright',legend=c('CBPM','VGPM','CAFE','VGPM_EP'),col=cols,lty=1,cex=0.6,bty='n')
mtext(paste(round(cor(chl[[6]],cbpm[[i]],use='pairwise.complete.obs'),3)),adj=0.7,cex=0.8,col=cols[1])	
...
mtext(paste(round(cor(chl[[6]],epvgpm[[i]],use='pairwise.complete.obs'),3)),adj=1,cex=0.8,col=cols[4])	

mtext(adj=0,labs[6],cex=0.6)
axis(1,labels=NA,at=ticks)
abline(v=ticks,lty=2)
axis(side=2)
plot(time,chl[[6]],type='l',xlim=timedims,xaxt='n',yaxt='n')
axis(1,labels=NA,at=ticks)
abline(v=ticks,lty=2)
axis(side=4)
for(i in 5:1){
  plot(time,cbpm[[i]],type='l',col=cols[1],xlim=timedims,xaxt='n',yaxt='n',ylim=range(c(cbpm[i],vgpm[i],cafe[i],epvgpm[i]),na.rm=TRUE)); 
  lines(time,vgpm[[i]],type='l',col=cols[2],xaxt='n',xlim=timedims,yaxt='n')
  lines(time,cafe[[i]],type='l',col=cols[3],xaxt='n',xlim=timedims,yaxt='n')
  lines(time,epvgpm[[i]],type='l',col=cols[4],xaxt='n',xlim=timedims,yaxt='n')
  abline(v=ticks,lty=2)
  axis(1,labels=NA,at=ticks)
  axis(side=2)
  par(new=TRUE)	
  ...
  axis(1,labels=NA,at=ticks)
  mtext(paste(round(cor(chl[[i]],cbpm[[i]],use='pairwise.complete.obs'),3)),adj=0.7,cex=0.8,col=cols[1])	
  ...
  mtext(paste(round(cor(chl[[i]],epvgpm[[i]],use='pairwise.complete.obs'),3)),adj=1,cex=0.8,col=cols[4])	
plot(time,chl[[i]],type='l',col='black',xlim=timedims,xaxt='n',yaxt='n'); 
axis(1,labels=NA,at=ticks)
abline(v=ticks,lty=2)
axis(side=4)
}
...
plot(time,cbpm[[6]],type='l',col='black',xaxt='n',xlim=timedims,yaxt='n')
	mtext(adj=0,labs[6],cex=0.6)
	axis(1,labels=NA,at=ticks)
	abline(v=ticks,lty=2)
	axis(side=4)
par(new=TRUE)	
plot(time,chl[[6]],type='l',xaxt='n',xlim=timedims,col='dark green',yaxt='n')
	mtext(paste(round(cor(chl[[6]],cbpm[[6]],use='pairwise.complete.obs'),3)),adj=1,cex=0.6)	
	axis(side=2,col='dark green')
for(i in 5:1){
	plot(time,cbpm[[i]],type='l',col='black',xlim=timedims,xaxt='n',yaxt='n'); 
	abline(v=ticks,lty=2)
	axis(1,labels=NA,at=ticks)
	axis(side=4)
par(new=TRUE)	
plot(time,chl[[i]],type='l',col='dark green',xaxt='n',xlim=timedims,yaxt='n')
	mtext(adj=0,labs[i],cex=0.6)
	axis(1,labels=NA,at=ticks)
	mtext(paste(round(cor(chl[[i]],cbpm[[i]],use='pairwise.complete.obs'),3)),adj=1,cex=0.6)	
	axis(side=2,col='dark green')
}
axis(1,at=ticks)
...
par(mfrow=c(6,1),mar=c(1,2,1,2),oma=c(2,3,2,3),cex.axis=0.7)
plot(time,vgpm[[6]],type='l',col='black',xaxt='n',xlim=timedims,yaxt='n')
mtext(adj=0,labs[6],cex=0.6)
axis(1,labels=NA,at=ticks)
abline(v=ticks,lty=2)
axis(side=4)
...
axis(side=2,col='dark green')
for(i in 5:1){
  plot(time,vgpm[[i]],type='l',col='black',xlim=timedims,xaxt='n',yaxt='n'); 
  abline(v=ticks,lty=2)
  axis(1,labels=NA,at=ticks)
  axis(side=4)
  par(new=TRUE)	
...
plot(time,chl[[6]],type='l',col='dark green',yaxt='n',xlim=timedims,xaxt='n',ylim=c(0,3)); 
axis(side=2,col='dark green'); 
axis(1,labels=NA,at=ticks)
par(new=TRUE)
plot(time,carbon[[6]],type='l',col='black',xaxt='n',yaxt='n',xlim=timedims)
axis(4)
abline(v=ticks,lty=2)
mtext(adj=0,labs[6],cex=0.6)
mtext(paste(round(cor(chl[[6]],carbon[[6]],use='pairwise.complete.obs'),3)),adj=1,cex=0.6)
for(i in 5:1){
  plot(time,chl[[i]],type='l',col='dark green',yaxt='n',xlim=timedims,xaxt='n'); 
  axis(side=2,col='dark green'); 
  axis(1,labels=NA,at=ticks)
  
  par(new=TRUE)
  plot(time,carbon[[i]],type='l',col='black',xaxt='n',yaxt='n',xlim=timedims)
  axis(4,col='black')
  abline(v=ticks,lty=2)
  mtext(adj=0,labs[i],cex=0.6)
...
days <- interval(ymd('1990-01-01'),date_decimal(time))
...
for(j in 1:6){
  for(i in 0:16){
    ...
    t_chl <- time[ii][chl[[j]][ii]     ==max(chl[[j]][ii],na.rm=TRUE)      & !is.na(chl[[j]][ii])]
    t_c   <- time[ii][carbon[[j]][ii]  ==max(carbon[[j]][ii],na.rm=TRUE)   & !is.na(carbon[[j]][ii])]
    lag[i+1,j] <- (t_chl-t_c)
  }
}
...
for(i in 6:1){ 
  plot(e[[i]],chlc[[i]],pch=19,cex=0.5,col=cols[month(date)],xlim=c(0,300),ylim=c(0,0.05))
...
plot(mld[[6]],carbon[[6]],pch=19,cex=0.25,xlim=c(0,150),ylim=c(0,70),xaxt='n',xlab='',ylab='')
linep(mld[[6]],carbon[[6]],mldin)
...
plot(mld[[6]],chl[[6]],pch=19,cex=0.25,xlim=c(0,150),ylim=c(0,2),xaxt='n',xlab='',ylab='')
    ...
    mtext(adj=0,cex=0.6,labs[2])
		k = 3
for(i in 5:1){
	plot(mld[[i]],carbon[[i]],pch=19,cex=0.25,xlim=c(0,150),ylim=c(0,40),xaxt='n',xlab='',ylab='')
		linep(mld[[i]],carbon[[i]],mldin)
		if(i==1) axis(1)
